...
int f; //температура2
int p; //давление
int humidity; // variable to store hemidity
...
void AQIGet() {
  ...
  /// если подключились, отправляем чего от сервера хотим
  // сам GET запрос с ID и ключем
  client.println("GET /data/2.5/air_pollution?lat=52.243949&lon=104.253271&appid=e9c015f4b101ce12f7d6ef877380cf27 HTTP/1.1");
  // говорим к какому хосту обращаемся (на сервере может быть несколько сайтов)
  client.println("Host: api.openweathermap.org");
  // говорим что закончили
  client.println("Connection: close");
